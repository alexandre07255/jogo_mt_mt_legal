...
void LevelSave::loadLevel(bool level2) {
	...
	if (level2) {
		...
		level->createPlayer1(saveJson["player1"]["x"], saveJson["player1"]["y"], saveJson["player1"]["points"]);

		if (isPlayer2) {
			level->createPlayer2(saveJson["player2"]["x"], saveJson["player2"]["y"], saveJson["player2"]["points"]);
		}
